...
from backend.db.database import init_db, AsyncSessionLocal, close_db, get_sync_session
...
def _send_pending_trade_telegram_alert(user_id: int, trade_data: dict):
    ...
    Uses sync DB session — safe to call from any thread / async context.
    """
    try:
...
def _handle_approve_pending_trade(user_id: int, payload: dict,
    ...
    if main_loop is not None:
        ...
        return {"ok": False, "error": "Worker main loop not available"}
...
def _expire_stale_pending_trades():
    ...
    try:
        ...
        if _command_loop_main_loop is not None:
            ...
            _log("expire_stale: main loop not available, skipping")
    except Exception as e:
